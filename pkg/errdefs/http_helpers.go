...
func GetHTTPErrorStatusCode(err error) int {
	if err == nil {
		logger.With(logger.Fields{"error": err}).Error("unexpected HTTP error handling")
	...
	case IsSystem(err) || IsUnknown(err) || IsDeadline(err) || IsCancelled(err):
...
func FromStatusCode(err error, statusCode int) error {
	...
	case http.StatusInternalServerError:
		if !IsSystem(err) && !IsUnknown(err) && !IsDeadline(err) && !IsCancelled(err) {
			err = System(err)
		}
	default:
		logger.With(logger.Fields{
